	...
	var interval = () =>
		setInterval(() => {
			...
			if (distance < 0) {
				...
				document.querySelector('.title-next-event-countdown').innerHTML =
					'EXPIRED';
				document.querySelector('.title-next-event-date').innerHTML = 'EXPIRED';
			} else {
				document.querySelector('.title-next-event-date').innerHTML =
					new Intl.DateTimeFormat('en-US', {
						dateStyle: 'full',
						timeStyle: 'long',
						timeZone: 'America/Los_Angeles',
					}).format(countDownDate);
				document.querySelector('.title-next-event-countdown-days').innerHTML =
					days;
				document.querySelector('.title-next-event-countdown-hours').innerHTML =
					hours;
				document.querySelector(
					'.title-next-event-countdown-minutes',
				).innerHTML = minutes;
				document.querySelector(
					'.title-next-event-countdown-seconds',
				).innerHTML = seconds;
			}
		});
